...
# Open the serial port for reading (COM5) and for writing (COM7)
with serial.Serial("COM5", 4800, timeout=1) as ser_input, serial.Serial(
    "COM7", 4800, timeout=1
) as ser_output:
    while True:
        # Read a line from the input serial port
        line = ser_input.readline().decode("ascii", errors="replace").strip()
        ...
        if line.startswith("$WIXDR"):
            ...
            # Write the formatted string to the output serial port
            ser_output.write((formatted_str + "\r\n").encode("ascii"))
        else:
            # Print other lines as-is
            print(line)

            # Write the original line to the output serial port
            ser_output.write((line + "\r\n").encode("ascii"))
